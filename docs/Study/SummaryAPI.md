...
- 기존 포스트 수정 시 -> 기존 요약본 수정, 포스트 업데이트 (No)
    - 수정한 모든 포스트를 인식 (기존, 신규 포함)
    - 이 포스트에 이미 요약본(TL;DR;)이 있는지 확인
    - 있으면 filtering out 후 새로운 요약본 붙임
- 기존에 이미 작성한 포스트에 대해 -> 아무런 액션 없음
    - 정확히는 기존에 작성했지만, 수정 안 한 포스트에 대해선 그냥 내비둠
- 포스트에 텍스트 외의 데이터가 포함된 경우 -> 포스트 데이터에서 텍스트만 필터링 (No)
    - 어차피 이미지 밖에 없고, 그 이미지도 url과 같은 텍스트 데이터라서 gpt 모델에서 알아서 거를 듯 싶음
- 요약본을 안 붙이고 싶은 경우
    - API 안 띄우면 summary yml 에러 발생, 내 gmail로 에러 알림 날아오고 요약본은 안 붙여짐
...
- api 서버 끄는건 VSC 종료하면 알아서 셧다운 됨 (좀 더 건전한(?) 방법?)
- pull 안하면 로컬, 원격 버전 달라져서 문제 생김
    - ```
    git fetch origin
